...
# ── Fleet definition ──────────────────────────────────────────────────────────
# Each entry: Name, CPUs, MemGB, DiskGB, Role
$Fleet = @(
    [PSCustomObject]@{ Name='lb-node';        CPUs=1; MemGB=1;  DiskGB=5;  Role='load-balancer' }
    [PSCustomObject]@{ Name='compute-node-1'; CPUs=2; MemGB=4;  DiskGB=10; Role='compute'       }
    [PSCustomObject]@{ Name='compute-node-2'; CPUs=2; MemGB=4;  DiskGB=10; Role='compute'       }
    [PSCustomObject]@{ Name='data-node-1';    CPUs=1; MemGB=2;  DiskGB=20; Role='data'          }
    [PSCustomObject]@{ Name='data-node-2';    CPUs=1; MemGB=2;  DiskGB=20; Role='data'          }
)
$UbuntuImage = '22.04'
...
# ══════════════════════════════════════════════════════════════════════════════
# HELPER: run a command and return stdout; throw on non-zero exit
# ══════════════════════════════════════════════════════════════════════════════
function Invoke-Cmd {
...
foreach ($vm in $Fleet) {
    Write-Host ('  {0,-22} {1,-8} {2,-8} {3,-10} {4}' -f `
        $vm.Name, "$($vm.CPUs) vCPU", "$($vm.MemGB) GB", "$($vm.DiskGB) GB", $vm.Role) -ForegroundColor White
}
Write-Host ''
...
foreach ($vm in $Fleet) {
    ...
    if ($existing) {
        Write-Warn "$($vm.Name) already exists (state: $existing) — skipping"
        $Skipped++
        continue
    }

    Write-Log "Launching $($vm.Name)  ($($vm.CPUs) vCPU · $($vm.MemGB)G RAM · $($vm.DiskGB)G disk)..."
    try {
        $launchArgs = @(
            'launch', $UbuntuImage,
            '--name',   $vm.Name,
...
Write-Ok "Fleet launch complete  (launched=$Launched, skipped=$Skipped)"
...
echo "[vm-bootstrap] Installing python3 + pip + stress-ng + sysstat..."
sudo apt-get install -y -qq python3 python3-pip stress-ng sysstat
...
foreach ($vm in $Fleet) {
    ...
    try {
        $raw = & multipass exec $vm.Name -- python3 -c $MetricSnippet 2>$null
...
foreach ($vm in $Fleet) {
    ...
    Write-Host ('  {0,-22} {1,-8} {2,-8} {3,-10} {4,-16} {5}' -f `
        $vm.Name, "$($vm.CPUs) vCPU", "$($vm.MemGB) GB", "$($vm.DiskGB) GB", $vm.Role, $ip) -ForegroundColor White
}
